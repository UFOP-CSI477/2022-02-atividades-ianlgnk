...
import { LocalColetaService } from "./service";

const service = new LocalColetaService();

export class LocalColetaController implements Controller {
  ...
  async findByName(request: Request, response: Response): Promise<Response> {
    try {
      ...
      if (name === "" || name === undefined || name === null)
        throw new APIError("Name is required to search a register!", 400);
